...
happens(signal_gate_closed, T) :- holdsAt(gate_angle(90), T, gate_is_lowering). %! holdsAt/3

happens(signal_gate_start_raising, T) :- !spy, happens(signal_train_exited, T).
happens(signal_gate_open, T) :- !spy, holdsAt(gate_angle(0), T, gate_is_rising).      %! holdsAt/3
