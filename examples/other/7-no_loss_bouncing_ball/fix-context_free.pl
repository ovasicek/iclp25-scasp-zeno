...
% falling 
can_trajectory(falling, T1, height(XatT2), T2) :-
    initiallyP(height(InitialX)),
    initiallyP(constant_fall_rate(Rate)),
    XatT2 .=. InitialX - ((T2-T1) * Rate).
trajectory(falling, T1, height(XatT2), T2) :-
    initiallyP(constant_fall_rate(Rate)),
    XatT2 .=. XatT1 - ((T2-T1) * Rate),
    holdsAt(height(XatT1), T1).

can_trajectory(falling, T1, velocity(XatT2), T2) :-
    initiallyP(velocity(InitialX)),
    initiallyP(constant_fall_rate(FallRate)),
    XatT2 .=. InitialX + ((T2-T1) * FallRate).
...
% rising
can_trajectory(rising, T1, height(XatT2), T2) :-
    initiallyP(constant_fall_rate(Rate)),
    XatT2 .=. 0 + ((T2-T1) * Rate).
...
can_trajectory(rising, T1, velocity(XatT2), T2) :-
    initiallyP(height(InitialX)),
    initiallyP(constant_fall_rate(FallRate)),
    XatT2 .=. InitialX - ((T2-T1) * FallRate).
