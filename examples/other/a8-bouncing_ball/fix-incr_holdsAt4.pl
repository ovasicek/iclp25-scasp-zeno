...
happens(bounce_up, T) :- incr_happens(hit_ground, T),
    minDuration(Dur), MinDur .>. Dur,
    holdsAt(height(0), T, falling, MinDur).
happens(stop_bouncing, T) :- incr_happens(hit_ground, T),
    minDuration(Dur), MaxDur .=<. Dur,
    holdsAt(height(0), T, falling, MaxDur).
...
?- happens(hit_ground,                  T).     % 20, 25, 26.25
