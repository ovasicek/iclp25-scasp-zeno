...
#include '../../axioms/general_utils.pl'.
...
fluent(constant_velocity_loss_bounce(X)).   % a flat amount of velocity lost at every bounce (heat, squish of the ball,...)
...
trajectory(rising, T1, velocity(XatT2), T2) :-
    ...
    XatT1FlatLoss = XatT1 - FlatBounceLoss,
    max(0, XatT1FlatLoss, XatT1FlatLossCropped),    % will never actually be less than zero due to trigger condition of event bounce_up
    % lose velocity on the way up
    initiallyP(constant_fall_rate(FallRate)),
    initiallyP(velocity_loss_portion(LossPortion)),
    StartVelocity .=. XatT1FlatLossCropped*(1-LossPortion),
...
happens(bounce_up, T) :- incr_happens(hit_ground, T),
    % only bounce up if there is enough velocity (more velocity than the flat loss from bounce)
    initiallyP(constant_velocity_loss_bounce(FlatBounceLoss)),
    Velocity .>. FlatBounceLoss,
    holdsAt(velocity(Velocity), T, falling).
happens(stop_bouncing, T) :- incr_happens(hit_ground, T),
    % stop bouncing if the is not enough velocity (less velocity than the flat loss from bounce)
    initiallyP(constant_velocity_loss_bounce(FlatBounceLoss)),
    Velocity .=<. FlatBounceLoss,
    holdsAt(velocity(Velocity), T, falling).
...
?- !incr_max_time(25),
   happens(hit_ground,                  T).     % 20, 24.5 (49/2), 25.125 (201/8)
