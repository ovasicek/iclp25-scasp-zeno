...
cat $SOURCE_FILES | grep -v " %//NO_PREPROCESS" | grep -E "^ *(initiates|terminates|releases).*" | sed -E "s| *:-.*$|.|" | sed -E "s/(initiates|terminates|releases)/can_\1/" >> $MODEL_NAME-preprocessed.pl

# create "can_trajectory(fluent1, T1, fluent2, T2)." for rules like "trajectory(fluent1, T1, fluent2, T2) :- holdsAt(...), happens(...), ... ."
cat $SOURCE_FILES | grep -v " %//NO_PREPROCESS" | grep -E "^ *trajectory.*:-" | sed -E "s| *:-.*$|.|" | sed -E "s|trajectory|can_trajectory|" >> $MODEL_NAME-preprocessed.pl
