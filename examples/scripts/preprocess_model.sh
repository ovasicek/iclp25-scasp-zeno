...
cat $SOURCE_FILES | grep -v " %//NO_PREPROCESS" | grep -E "^ *(initiates|terminates|releases).*\." | sed -E "s| *:-.*$|.|" | sed -E "s/(initiates|terminates|releases)/can_\1/g" >> $MODEL_NAME-preprocessed.pl
